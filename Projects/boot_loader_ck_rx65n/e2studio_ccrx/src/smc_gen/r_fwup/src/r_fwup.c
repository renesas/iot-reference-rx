...
#define FWUP_COPY_BUF_SIZE                     (128U)
...
#define FWUP_BUF_AREA_NUM_BLKS                 (FWUP_CFG_AREA_SIZE / FWUP_BUF_AREA_BLK_SIZE)
#endif /* (FWUP_CFG_UPDATE_MODE s= FWUP_SINGLE_BANK_W_BUFFER_EXT) */
...
S_C_CH_FAR MSG_VERIFY_INSTALL_AREA[] = "verify install area %d [%s]...";
 ...
 * Description  : Check user program
 * Arguments    : area      MAIN_AREA/BUFFER_AREA
 * Return Value : true      User program found
 *                false     User program not found
 *********************************************************************************************************************/
 ...
 * Description  : Erasing Install area.
 * Return Value : FWUP_SUCCESS
 *                FWUP_ERR_FLASH
 *********************************************************************************************************************/
 ...
 * Description  : Get user program size
 * Arguments    : None
 * Return Value : 0     : header not received yet.
 *                other : user program size
 *********************************************************************************************************************/
uint32_t R_FWUP_GetImageSize(void)
{
    /* Received header? */
    if (sizeof(st_fw_ctrlblk_t) > s_image_size)
    {
        return 0;
    }
    return s_image_size;
 ...
 * Description  : Write image file header to Buffer area.
 * Arguments    : area        MAIN_AREA/BUFFER_AREA
                                     ...
                                     uint8_t FWUP_FAR *p_sig, uint32_t sig_size)
{
    ...
    if (true == check_magic_code(FWUP_AREA_BUFFER))
    {
        return FWUP_SUCCESS;
    }
 ...
 * Description  : Activate New user program
 * Arguments    : None
 * Return Value : none
 *********************************************************************************************************************/
 ...
 * Description  : Jump to user program
 * Arguments    : None
 * Return Value : None
 *********************************************************************************************************************/
...
* Description  : Get offset address from top of install area.
* Arguments    : code flash address
* Return Value : offset from install area
**********************************************************************************************************************/
static bool check_addr_range(uint32_t addr)
{
...
* Arguments    : area  MAIN_AREA/BUFFER_AREA/DATA_FLASH
* Return Value : FWUP_SUCCESS
*                FWUP_ERR_FLASH
**********************************************************************************************************************/
static e_fwup_err_t erase_area(e_fwup_area_t area)
 ...
 * Arguments    : area      MAIN_AREA/BUFFER_AREA/DATA_FLASH
 *                p_buf:(i/o)     firmware data
 *                p_bufsz:(i/o)  firmware data size
 *                offset         write offset from install area. (not flash address!!!)
 *                size           write size
 *                left_sz   number of not flashed data.
...
* Arguments    : area       MAIN_AREA/BUFFER_AREA/DATA_FLASH
*                pbuf       buffer pointer
*                read_addr  MAIN_AREA/BUFFER_AREA:offset from install area.
*                           DATA_FLASH:flash address
*                size       read size
* Return Value : none
**********************************************************************************************************************/
static void read_area(e_fwup_area_t area, uint32_t * p_buf, uint32_t area_offset, uint32_t size)
 ...
 * Description  : Copy temporary area to main area
 * Arguments    : None
 * Return Value : FWUP_SUCCESS
 *                FWUP_ERR_FLASH
 *********************************************************************************************************************/
static e_fwup_err_t copy_to_main_area(void)
{
    ...
    int32_t     block_cnt;
    uint32_t    max_cnt;
    uint32_t    cf_block[FWUP_COPY_BUF_SIZE / 4];
...
static int32_t sha256_update(e_fwup_area_t area, void * vp_ctx, uint32_t area_offset, uint32_t datalen)
{
    uint32_t rbuf[FWUP_READ_BUF_SIZE / 4];
 ...
 * Description  : calculate SHA256 hash on designated install area. (MAIN_AREA or BUFFER_AREA)
 * Arguments    : area     MAIN_AREA/BUFFER_AREA
 * Return Value : pointer of hash data.
 *********************************************************************************************************************/
static uint8_t * hash_sha256(e_fwup_area_t area)
{
    static uint8_t puc_hash[32];
    uint32_t area_offset;
    st_fw_desc_t dc;
