 ...
 */
IPV4Parameters_t xNd = {RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE, RESET_VALUE};
 ...
 * Return Value : void
 *********************************************************************************************************************/
static void copy_xNd_to_ip_arrays(void)
{
    ucIPAddress[0] = (uint8_t)(xNd.ulIPAddress & 0xFF);
    ucIPAddress[1] = (uint8_t)((xNd.ulIPAddress >> 8) & 0xFF);
    ucIPAddress[2] = (uint8_t)((xNd.ulIPAddress >> 16) & 0xFF);
    ucIPAddress[3] = (uint8_t)((xNd.ulIPAddress >> 24) & 0xFF);

    ucNetMask[0] = (uint8_t)(xNd.ulNetMask & 0xFF);
    ucNetMask[1] = (uint8_t)((xNd.ulNetMask >> 8) & 0xFF);
    ucNetMask[2] = (uint8_t)((xNd.ulNetMask >> 16) & 0xFF);
    ucNetMask[3] = (uint8_t)((xNd.ulNetMask >> 24) & 0xFF);

    ucGatewayAddress[0] = (uint8_t)(xNd.ulGatewayAddress & 0xFF);
    ucGatewayAddress[1] = (uint8_t)((xNd.ulGatewayAddress >> 8) & 0xFF);
    ucGatewayAddress[2] = (uint8_t)((xNd.ulGatewayAddress >> 16) & 0xFF);
    ucGatewayAddress[3] = (uint8_t)((xNd.ulGatewayAddress >> 24) & 0xFF);

    ucDNSServerAddress[0] = (uint8_t)(xNd.ulDNSServerAddresses[0] & 0xFF);
    ucDNSServerAddress[1] = (uint8_t)((xNd.ulDNSServerAddresses[0] >> 8) & 0xFF);
    ucDNSServerAddress[2] = (uint8_t)((xNd.ulDNSServerAddresses[0] >> 16) & 0xFF);
    ucDNSServerAddress[3] = (uint8_t)((xNd.ulDNSServerAddresses[0] >> 24) & 0xFF);
}
/**********************************************************************************************************************
...
void net_thread_entry(void *pvParameters)
{
    ...
    #if( ipconfigUSE_DHCP != 0 )
    /* Convert xNd fields to byte arrays before calling FreeRTOS_IPInit */
    copy_xNd_to_ip_arrays();
...
void print_ipconfig(void)
{
    ...
    if (g_dhcp_in_use)
    {
        ...
        ucDNSServerAddress[3] = (uint8_t)((xNd.ulDNSServerAddresses[ 0 ] & 0xFF000000) >> 24); // Cast to uint8_t
        ucDNSServerAddress[2] = (uint8_t)((xNd.ulDNSServerAddresses[ 0 ] & 0x00FF0000) >> 16); // Cast to uint8_t
        ucDNSServerAddress[1] = (uint8_t)((xNd.ulDNSServerAddresses[ 0 ] & 0x0000FF00) >> 8);  // Cast to uint8_t
        ucDNSServerAddress[0] = (uint8_t)(xNd.ulDNSServerAddresses[ 0 ] & 0x000000FF);         // Cast to uint8_t

        ucIPAddress[3]        = (uint8_t)((xNd.ulIPAddress & 0xFF000000) >> 24); // Cast to uint8_t
        ucIPAddress[2]        = (uint8_t)((xNd.ulIPAddress & 0x00FF0000) >> 16); // Cast to uint8_t
        ucIPAddress[1]        = (uint8_t)((xNd.ulIPAddress & 0x0000FF00) >> 8);  // Cast to uint8_t
        ucIPAddress[0]        = (uint8_t)(xNd.ulIPAddress & 0x000000FF);         // Cast to uint8_t
    }
#endif
...
void update_dhcp_response_to_usr(void)
{
    if (g_dhcp_in_use)
    {
    	xNd.ulIPAddress = FreeRTOS_GetIPAddress();
