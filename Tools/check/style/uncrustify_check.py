...
# ----------------------------
# Run uncrustify on a file
# ----------------------------
def run_uncrustify(file_path: Path, uncrust_conf_path: Path):
    file_path_str = str(file_path.resolve().as_posix())
    uncrust_conf_path = str(uncrust_conf_path.resolve().as_posix())

    # Hardcoded uncrustify path
    uncrustify_path = r"C:\tools\uncrustify-0.81.0_f-win64\bin\uncrustify.exe"

    # Output to .uncrustified
    out_path = file_path.with_suffix(file_path.suffix + ".uncrustified")
    out_path_str = str(out_path)

    cmd = [uncrustify_path, "-c", uncrust_conf_path, "--no-backup", "-f", file_path_str]
    ...
    try:
        ...
        logger.error(f"!! Error: uncrustify.exe not found at {uncrustify_path}")
        return 1, ""
...
def check_file(file_path: Path, diff_root: Path, base_folder: Path, fix: bool, style_file: Path, packages_path: Path) -> bool:
    ...
    retcode, formatted_code = run_uncrustify(file_path, style_file)

    if retcode != 0:
        logger.error(f"!! run_uncrustify failed with return code {retcode} for {file_path}")
        return False
    ...
    if original_code != formatted_code:
        if fix:
            ...
            diff = difflib.unified_diff(
                ...
                tofile=str(file_path) + ".uncrustified",
                lineterm=""
            )
            diff_text = "".join(diff)
...
def main():
    ...
    uncrust_conf_path = curr_path / 'uncrust_conf.json'
    if not uncrust_conf_path.exists():
        logger.error(f"!! uncrust_conf.json not found: {uncrust_conf_path}")
        sys.exit(1)
    uncrust_config = read_json(uncrust_conf_path)

    extensions = uncrust_config.get("file_extensions", [".c", ".h"])
    exclude = set(uncrust_config.get("ExcludePath", []))
    fix_uncrustify = uncrust_config.get("fix", False)

    diff_root = top_path / 'diff_output'

    # Resolve uncrustify.cfg file
    style_file = curr_path / 'uncrustify.cfg'
    if not style_file or not style_file.exists():
        logger.error(f"!! uncrustify.cfg not found: {style_file}")
        sys.exit(1)

    if not fix_uncrustify:
        if diff_root.exists():
    ...
    if not folder_path.exists() or not folder_path.is_dir():
        ...
        ok, checked, changed = check_folder_recursive(
            folder_path, set(extensions), diff_root, fix_uncrustify, style_file, top_path, exclude
        )
    ...
    logger.info("\n---- SUMMARY ----")
    logger.info(f"Mode: {'FIX' if fix_uncrustify else 'CHECK'}")
    logger.info(f"Files checked: {total_checked}")
    if not fix_uncrustify:
        logger.info(f"Files needing changes: {total_changed}")

    sys.exit(0 if fix_uncrustify or overall_ok else 1)
